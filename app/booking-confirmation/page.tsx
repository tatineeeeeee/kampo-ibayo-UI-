...
function BookingConfirmationContent() {
  ...
  const getStatusIcon = () => {
    switch (paymentStatus) {
      ...
      default:
        return <Loader2 className="w-20 h-20 text-muted-foreground drop-shadow-lg" />;
    }
  };
    ...
    return (
      <div className="min-h-screen relative flex items-center justify-center">
        ...
        <div className="relative text-center">
          ...
          <p className="text-foreground text-base sm:text-lg">Loading booking details...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen relative flex items-center justify-center p-4">
        ...
        <div className="relative bg-white/10 backdrop-blur-xl rounded-2xl shadow-2xl p-6 sm:p-8 max-w-md text-center border border-white/20">
          ...
          <button
            ...
            className="px-6 py-3 bg-gradient-to-r from-primary to-primary/90 text-foreground rounded-lg font-semibold hover:from-primary/90 hover:to-primary/80 transition-all duration-200 shadow-lg"
          >
            Back to Booking
          </button>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="text-foreground">
                ...
                <p className="text-xs sm:text-sm text-muted-foreground hidden sm:block">Booking Confirmation</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Link
                href="/bookings"
                className="px-3 py-2 text-sm text-muted-foreground hover:text-foreground bg-card hover:bg-muted rounded-lg transition-colors"
              >
                My Bookings
              </Link>
            </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white/10 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 mb-6 sm:mb-8">
            <div className={`p-4 sm:p-8 text-center rounded-t-2xl ${
              ...
              <h1 className="text-2xl sm:text-3xl font-bold mb-3 sm:mb-4 text-foreground">{getStatusTitle()}</h1>
              <p className="text-base sm:text-lg text-foreground max-w-2xl mx-auto leading-relaxed px-2">{getStatusMessage()}</p>
            </div>

            {/* Booking Details */}
            <div className="p-4 sm:p-8">
              <h2 className="text-xl sm:text-2xl font-bold text-foreground mb-6 sm:mb-8">Booking Details</h2>
            
            <div className="grid gap-4 sm:gap-6 lg:grid-cols-2">
              {/* Left Column */}
              <div className="space-y-4 sm:space-y-6">
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 sm:p-6 border border-white/20">
                  <h3 className="text-base sm:text-lg font-semibold text-foreground mb-3 sm:mb-4 flex items-center gap-2 sm:gap-3">
                    <div className="w-7 h-7 sm:w-8 sm:h-8 bg-primary/30 rounded-lg flex items-center justify-center">
                      <Users className="w-3 h-3 sm:w-4 sm:h-4 text-primary/80" />
                    </div>
                    Guest Information
                  </h3>
                  <div className="space-y-2 sm:space-y-3">
                    <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-black/20 rounded-lg">
                      <span className="text-muted-foreground text-xs sm:text-sm min-w-[50px] sm:min-w-[60px]">Name:</span>
                      <span className="text-foreground font-medium text-sm sm:text-base">{booking.guest_name}</span>
                    </div>
                    <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-black/20 rounded-lg">
                      <Mail className="w-3 h-3 sm:w-4 sm:h-4 text-muted-foreground flex-shrink-0" />
                      <span className="text-foreground text-xs sm:text-sm break-all">{booking.guest_email || 'Not provided'}</span>
                    </div>
                    <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-black/20 rounded-lg">
                      <Phone className="w-3 h-3 sm:w-4 sm:h-4 text-muted-foreground flex-shrink-0" />
                      <span className="text-foreground text-xs sm:text-sm">{displayPhoneNumber(booking.guest_phone) || 'Not provided'}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 sm:p-6 border border-white/20">
                  <h3 className="text-base sm:text-lg font-semibold text-foreground mb-3 sm:mb-4 flex items-center gap-2 sm:gap-3">
                    <div className="w-7 h-7 sm:w-8 sm:h-8 bg-primary/30 rounded-lg flex items-center justify-center">
                      <Calendar className="w-3 h-3 sm:w-4 sm:h-4 text-primary/80" />
                    </div>
                    Stay Details
                  </h3>
                  <div className="space-y-2 sm:space-y-3">
                    <div className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-black/20 rounded-lg border border-white/10">
                      ...
                      <div>
                        ...
                        <p className="text-foreground font-semibold text-sm sm:text-base">{formatDate(booking.check_in_date)}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-black/20 rounded-lg border border-white/10">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 bg-primary/30 rounded-lg flex items-center justify-center flex-shrink-0">
                        <Calendar className="w-4 h-4 sm:w-5 sm:h-5 text-primary/80" />
                      </div>
                      <div>
                        <p className="text-muted-foreground text-xs sm:text-sm">Check-out</p>
                        <p className="text-foreground font-semibold text-sm sm:text-base">{formatDate(booking.check_out_date)}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 p-4 bg-card/40 rounded-lg border border-border/20">
                      <div className="w-10 h-10 bg-primary/20 rounded-lg flex items-center justify-center">
                        <Users className="w-5 h-5 text-primary" />
                      </div>
                      <div>
                        <p className="text-muted-foreground text-sm">Total Guests</p>
                        <p className="text-foreground font-semibold">{booking.number_of_guests} {booking.number_of_guests === 1 ? 'guest' : 'guests'}</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-6">
                <div className="bg-muted/30 backdrop-blur-sm rounded-xl p-6 border border-border/30">
                  ...
                  <div className="space-y-3">
                    ...
                    {booking.payment_intent_id && (
                      <div className="flex items-center justify-between p-3 bg-card/30 rounded-lg">
                        ...
                        <span className="font-mono text-sm text-foreground">{booking.payment_intent_id.slice(-8)}</span>
                      </div>
                    )}
                    <div className="flex items-center justify-between p-3 bg-card/30 rounded-lg">
                    ...
                    <div className="border-t border-border/30 pt-4 mt-4">
                      <div className="flex items-center justify-between text-xl font-bold p-4 bg-gradient-to-r from-primary/10 to-primary/90/10 rounded-lg border border-primary/20">
              ...
              {(paymentStatus === 'success' || paymentStatus === 'pending') && booking && (
                <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-white/20">
                  <div className="bg-white/5 backdrop-blur-sm rounded-xl p-4 sm:p-6 border border-white/10">
                    <h3 className="text-lg sm:text-xl font-semibold text-foreground mb-4 flex items-center gap-3">
                      <div className="w-7 h-7 sm:w-8 sm:h-8 bg-primary/30 rounded-lg flex items-center justify-center">
                        <Calendar className="w-3 h-3 sm:w-4 sm:h-4 text-primary/80" />
                      </div>
                      Cancellation Policy
              ...
              <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-white/20">
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
                  <button
                    ...
                    className="flex items-center justify-center px-4 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-primary to-primary/90 text-foreground rounded-lg font-semibold hover:from-primary/90 hover:to-primary/80 transition-all duration-200 shadow-lg text-sm sm:text-base"
                  >
                    View My Bookings
      ...
      <div className="min-h-screen relative">
        ...
        <div className="relative z-10 flex items-center justify-center min-h-screen">
          <div className="text-center">
            <Loader2 className="w-16 h-16 animate-spin text-primary mx-auto mb-4 drop-shadow-lg" />
            <p className="text-foreground text-xl font-semibold drop-shadow">Loading booking details...</p>
          </div>
        </div>
      </div>
    }>
