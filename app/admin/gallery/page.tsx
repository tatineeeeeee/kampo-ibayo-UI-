...
import { Upload, Image as ImageIcon, Trash2, Edit3, Eye, Star, StarOff } from "lucide-react";
...
function ImageUploadModal({ isOpen, onClose, onSuccess }: ImageUploadModalProps) {
...
function DeleteConfirmModal({ isOpen, onClose, onConfirm, type, imageName, totalImages, isDeleting = false }: DeleteConfirmModalProps) {
  if (!isOpen) return null;
      ...
      <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className={`p-2 rounded-lg ${type === 'all' ? 'bg-red-100' : 'bg-orange-100'}`}>
              <Trash2 className={`w-5 h-5 ${type === 'all' ? 'text-red-600' : 'text-orange-600'}`} />
            </div>
          ...
          {type === 'all' && (
            <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg">
              <p className="text-sm text-red-700 font-medium">
                ⚠️ This action cannot be undone. All images will be permanently deleted from both the database and storage.
              </p>
            </div>
          )}
        </div>
...
export default function GalleryPage() {
  ...
  const performDeleteAll = async () => {
    try {
      ...
      if (storageError) {
        ...
        console.warn('Images removed from database but some storage cleanup failed');
      }
  ...
  const toggleFeatured = async (image: GalleryImage) => {
    try {
      ...
      success(`Image ${!image.is_featured ? 'marked as featured' : 'removed from featured'}!`);
      fetchImages();
    } catch (error) {
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              {loading ? 'Loading gallery...' : `Manage resort gallery images (${images.length} total)`}
            </p>
          </div>
          <div className="flex gap-3">
            <button
              ...
              <div key={image.id} className="relative group bg-gray-100 rounded-lg overflow-hidden">
                ...
                <div className="p-3">
                  <p className="text-sm font-medium text-gray-900 truncate">
                    {image.caption || (image.file_name ? image.file_name.replace(/\.[^/.]+$/, '').replace(/-/g, ' ').replace(/_/g, ' ') : 'Gallery Image')}
                  </p>
